...
export default function QuoteModal() {
  ...
  // URL Backend : Utiliser le proxy Vite en dev (relative URL) ou l'URL directe en production
  const backendBase = ((import.meta.env as any).VITE_BACKEND_URL || '').trim();
  const sendQuoteUrl = backendBase && import.meta.env.MODE === 'production'
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="glass-card bg-dark-card rounded-2xl border border-white/10 max-w-2xl w-full max-h-[90vh] overflow-auto quote-modal-content">
        <div className="p-6">
          ...
          {showTemplate ? (
            <div className="bg-gradient-to-br from-green-50 to-blue-50 p-6 rounded-lg shadow-lg border border-green-200">
              ...
              <div className="flex items-center mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-green-600">Email envoyé à l'équipe Bedouielec Transformateur avec succès.</p>
                </div>
              </div>
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-6 bg-white/5 border border-white/10 p-5 rounded-xl">
                <h3 className="font-semibold text-white mb-3">Produits concernés:</h3>
                <div className="space-y-2">
                  {products.map((item: any, index: number) => (
                    <div key={index} className="flex justify-between items-center text-sm text-slate-300">
                      <span>{item.product.name}</span>
                      <span className="font-medium text-electric-blue">{item.totalPrice.toLocaleString()} TND</span>
                    </div>
                  ))}
                </div>
                <div className="border-t border-white/10 mt-3 pt-3">
                  <div className="flex justify-between items-center font-bold text-white">
                    ...
                    <span className="text-electric-blue">{totalPrice.toLocaleString()} TND</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-electric-blue/10 border border-electric-blue/20 p-4 rounded-xl mb-6">
                <p className="text-sm text-slate-300">
                  <strong className="text-electric-blue">Note:</strong> Nos experts vous contacteront dans les 24h avec un devis personnalisé incluant les prix, disponibilités et conditions de livraison.
                </p>
              </div>
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className={`w-full py-3 px-6 rounded-xl transition-all flex items-center justify-center space-x-2 ${
                  isSubmitting 
