...
export default function ProductModal({ product, isOpen, onClose }: ProductModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="glass-card bg-dark-card rounded-2xl max-w-2xl sm:max-w-3xl w-full max-h-[90vh] overflow-auto mx-auto border border-white/10 shadow-2xl">
        <div className="p-6">
            ...
            <div className="px-4 md:px-6 lg:px-8 flex flex-col items-center">
              <div className="mb-4">
                ...
                {((product.images && product.images.length > 0) || product.image) ? (
                              ...
                              {imagesArray.map((image: string, index: number) => (
                                <button
                                  ...
                                  className={`w-16 h-16 rounded-md overflow-hidden border-2 transition-all ${
                                    selectedImageIndex === index ? 'border-electric-blue' : 'border-white/10 opacity-50 hover:opacity-100'
                                  }`}
                                >
                  ...
                  <div className="w-full h-96 flex items-center justify-center bg-white/5 rounded-lg text-slate-500">
              ...
              <div className="w-full max-w-md text-center">
                <div className="flex items-center justify-center space-x-4 mb-4">
                  <span className="text-sm font-medium text-electric-blue bg-electric-blue/10 border border-electric-blue/20 px-3 py-1 rounded-lg">
                    {product.brand}
                  </span>
                  ...
                  <div className="flex items-center space-x-1">
                    ...
                    <span className="text-slate-500">(100+ avis)</span>
                  </div>
                </div>
                ...
                {/* Specifications */}
                <div className="mb-6">
                  <h3 className="text-lg font-semibold text-white mb-3">Caractéristiques techniques</h3>
                      ...
                      {Object.entries(product.specifications || product.characteristics || {}).map(([key, value]) => (
                        <div key={key} className="py-2 w-full max-w-sm">
                          <div className="flex items-center gap-3 justify-center text-center">
                            <span className="font-medium text-slate-300 whitespace-nowrap">{key}:</span>
                ...
                <div className="mb-6 text-center">
                  <label className="block text-sm font-medium text-slate-400 mb-2">Quantité</label>
                  <div className="flex items-center justify-center space-x-3 mt-2">
                    <button
                      onClick={() => setQuantity(Math.max(1, quantity - 1))}
                      className="p-2 border border-white/10 rounded-md hover:bg-white/5 text-slate-300"
                      aria-label="Réduire la quantité"
                    >
                      <Minus className="w-4 h-4" />
                    </button>
                    <span className="font-medium text-lg px-4 text-white">{quantity}</span>
                    <button
                ...
                <div className="flex space-x-4">
                    <button
                      ...
                      className="flex-1 btn-primary py-3 px-6 rounded-xl flex items-center justify-center space-x-2"
                    >
                ...
                <button
                  ...
                  className="w-full mt-3 btn-orange py-3 px-6 rounded-xl"
                >
                  Demander un devis
                </button>
              </div>
