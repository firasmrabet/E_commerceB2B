...
const initialState = {
  ...
  userEmail: null,
  userProfile: null,
  isProfileComplete: true, // par défaut true, sera vérifié au login
};
  ...
  const adminEmails = ['firassmrabett111@gmail.com', 'marwenyoussef2017@gmail.com'];
    ...
    case 'SET_USER_PROFILE':
      return {
        ...
        isProfileComplete: action.payload?.isComplete ?? true,
      };
    default:
      return state;
  }
...
function AppProvider({ children }) {
  ...
  const checkProfileStatus = async (user) => {
    ...
    const deploymentDate = new Date('2026-07-07T00:00:00Z');
    
    // Ancien compte ? On ignore la vérification
    if (createdAt < deploymentDate) {
      dispatch({ type: 'SET_USER_PROFILE', payload: { profile: null, isComplete: true } });
      return;
    }
    
    // Nouveau compte : on fetch via notre API
    try {
      const res = await fetch(`/api/get-profile/${user.id}`);
      if (res.ok) {
        const data = await res.json();
        dispatch({ type: 'SET_USER_PROFILE', payload: { profile: { name: data.name, phone: data.phone }, isComplete: data.isComplete } });
      } else {
        dispatch({ type: 'SET_USER_PROFILE', payload: { profile: null, isComplete: false } });
      }
    } catch (e) {
  ...
  useEffect(() => {
    async function fetchUser() {
      ...
      if (data?.user) {
        ...
        await checkProfileStatus(data.user);
    ...
    fetchUser();
    const { data: unsubscribe } = supabase.auth.onAuthStateChange(async (_event, session) => {
      if (session?.user) {
        ...
        try {
          ...
          const norm = typeof raw === 'string' ? raw.toLowerCase().trim() : '';
          const adminEmails = ['firassmrabett111@gmail.com', 'marwenyoussef2017@gmail.com'];
          console.log('[Auth Debug] onAuthStateChange user email:', raw, 'normalized:', norm, 'isAdmin:', adminEmails.includes(norm));
        } catch (e) {}
      } else {
        setUserId(null);
        dispatch({ type: 'SET_USER_EMAIL', payload: null });
